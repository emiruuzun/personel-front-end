...
function JobAssignmentsList() {
  ...
  const fetchWorkRecords = useCallback(async () => {
    ...
    try {
      ...
      const workResponse = await getWorkRecordsByDateRange(
        formattedDate,
        formattedDate
      );
      const leaveResponse = await getAllLeave();

      if (workResponse) {
        setAssignedRecords(workResponse.assigned);
      }

      if (leaveResponse) {
        const filteredLeaves = leaveResponse.data.filter((leave) => {
  ...
  const filteredAssignedRecords = assignedRecords.filter((record) =>
    record.personnel_id.name.toLowerCase().includes(searchTerm.toLowerCase())
  );
      ...
      <div className="p-6 bg-gray-100 min-h-screen">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Personel iş atamalarını ve izin durumlarını kontrol edin.
          </p>
        </div>
            ...
            <div className="bg-white rounded-lg shadow-md overflow-hidden">
              ...
              <div className="px-6 py-4 bg-indigo-600 text-white flex items-center justify-between">
                ...
                <h2 className="text-xl font-semibold flex items-center">
                  {" "}
                  <FaUserTie className="mr-2" /> Atanmış İş Kayıtları{" "}
                </h2>{" "}
